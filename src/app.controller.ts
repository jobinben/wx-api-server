import { Controller, Get, Req, Res, Post, Body, HttpCode } from '@nestjs/common';
...
export class AppController {
  ...
  async getWxMsg(@Req() request: Request): Promise<any> {
	...
	const isPassed = await this.appService.checkSignature(signature, timestamp, nonce);
  ...
  @HttpCode(200)
  @Post('wxMsg')
  handleWxMsg(@Req() req, @Res() res){
	const xmlData = req.body.xml; // 获取已经解析后的 XML 数据
